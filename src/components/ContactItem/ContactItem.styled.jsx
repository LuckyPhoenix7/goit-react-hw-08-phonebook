...
export const DeleteButton = styled.button`
  background-color: ${p => p.theme.colors.error}; // Светлый красный
  color: ${p => p.theme.colors.background};
  border: none;
  padding: 5px 10px;
  border-radius: 5px;
  cursor: pointer;
  &:hover {
    background-color: ${p => p.theme.colors.hover}; // Розовато-коричневый
  }
`;
...
export const StyledContactIcon = styled(FiUser).attrs({
  ...
  margin-right: 10px;
  color: ${p => p.theme.colors.primary}; // Пыльная роза
`;

export const StyledDeleteContact = styled(FiUserX).attrs({
  size: '20px',
})`
  color: ${p => p.theme.colors.error}; // Светлый красный
`;
